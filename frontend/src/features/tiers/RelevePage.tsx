...
import { getReleve, type ReleveEntry } from "@/api/releve";
...
export default function RelevePage() {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["releve", id, dateDebut, dateFin],
    queryFn: () => getReleve(id, dateDebut, dateFin),
    enabled: !!id,
  });
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <Space>
          <Link to="/tiers/clients">
            <Button icon={<ArrowLeftOutlined />}>Clients</Button>
          </Link>
          <Typography.Title level={3} style={{ margin: 0 }}>
            Relevé de compte {data?.client.nom ? `— ${data.client.nom}` : ""}
          </Typography.Title>
        </Space>
        <Space>
          <RangePicker
